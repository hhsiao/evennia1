        ...
        if "list" in switches or self.cmdstring in ("nicks",):
            if not nicklist:
                ...
                for inum, nickobj in enumerate(nicklist):
                    __, __, nickvalue, replacement = nickobj.value
                    table.add_row(
        ...
        if "delete" in switches or "del" in switches:
            ...
            if oldnicks:
                for oldnick in oldnicks:
                    ...
                    __, __, old_nickstring, old_replstring = oldnick.value
...
class CmdAccess(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        string += _("\n|wYour access|n:")
        string += _("\nCharacter |c{caller.key}|n: {cperms}").format(cperms=cperms, caller=caller)
        if utils.inherits_from(caller, evennia.DefaultObject):
            string += _("\nAccount |c{caller.account.key}|n: {pperms}").format(pperms=pperms, caller=caller)
        caller.msg(string)
